...
$Script:Strings = @{
    ...
    "FR" = @{
        ...
        MsgOpennessSecurityError = "Acces TIA Openness refuse (erreur de securite).`n`nLe compte Windows qui execute TIA Portal n'est pas membre du groupe local << Siemens TIA Openness >>.`n`nSolution (en administrateur) : ajoutez l'utilisateur a ce groupe via lusrmgr.msc, ou en ligne de commande :`n  net localgroup `"Siemens TIA Openness`" `"DOMAINE\Utilisateur`" /add`n`nFermez ensuite la session Windows (ou redemarrez) puis relancez TIA Portal.`n`nDetail :`n{0}"
    ...
    "EN" = @{
        ...
        MsgOpennessSecurityError = "TIA Openness access denied (security error).`n`nThe Windows account running TIA Portal is not a member of the local group << Siemens TIA Openness >>.`n`nFix (as administrator): add the user to that group via lusrmgr.msc, or from the command line:`n  net localgroup `"Siemens TIA Openness`" `"DOMAIN\User`" /add`n`nThen sign out of Windows (or reboot) and restart TIA Portal.`n`nDetails:`n{0}"
    ...
    "ES" = @{
        ...
        MsgOpennessSecurityError = "Acceso TIA Openness denegado (error de seguridad).`n`nLa cuenta de Windows que ejecuta TIA Portal no es miembro del grupo local << Siemens TIA Openness >>.`n`nSolucion (como administrador): anada el usuario a ese grupo mediante lusrmgr.msc, o por linea de comandos:`n  net localgroup `"Siemens TIA Openness`" `"DOMINIO\Usuario`" /add`n`nLuego cierre la sesion de Windows (o reinicie) y vuelva a iniciar TIA Portal.`n`nDetalle:`n{0}"
